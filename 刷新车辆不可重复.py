...
if run:
    ...
    # 构建每辆车的刷新权重：档位权重平均分配给该档位下的每辆车
    car_weights = np.zeros(total_cars)

    for tier in tiers:
        start_idx = tier_car_start_index[tier]
        count_in_tier = tier_car_count[tier]
        end_idx = start_idx + count_in_tier
        car_weights[start_idx:end_idx] = tier_refresh_weight[tier] / count_in_tier

    car_probs = car_weights / car_weights.sum()

    if cars_per_cycle > total_cars:
        st.error("每个玩家每周期刷到车辆数不能大于车辆池总车辆数，否则无法保证不重复。")
        st.stop()
    ...
    for cycle in range(1, simulate_cycles + 1):
        ...
        for seg in segments:
            ...
            # 每个玩家从完整车辆池中不放回抽样，保证同一周期不重复刷到同一辆车
            car_ids = np.zeros((player_count, cars_per_cycle), dtype=int)

            for i in range(player_count):
                car_ids[i] = rng.choice(
                    total_cars,
                    size=cars_per_cycle,
                    replace=False,
                    p=car_probs
            ...
            # 根据抽到的车辆反推档位
            tier_indices = np.zeros_like(car_ids)
            ...
            for tier in tiers:
                ...
                mask = (car_ids >= start_idx) & (car_ids < end_idx)
